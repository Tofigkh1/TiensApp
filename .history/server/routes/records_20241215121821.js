...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Check if user has an existing discount in Firestore
    const userDiscounts = await getQueryData("discount", "userId", userId);

    // Check if basket contains 4 or more unique items
    const uniqueItems = new Set(userBasket.items.map(item => item.productId));
    if (uniqueItems.size >= 4) {
      console.log(`Basket contains ${uniqueItems.size} unique items. Checking discount collection...`);

      // If discounts exist, delete them
