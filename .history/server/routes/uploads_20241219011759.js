...
export async function handlerGetAllVideos(req, res, folder) {
  try {
    ...
    const videosFolderRef = ref(storage, `${folder}/videos`);
    const files = await listAll(videosFolderRef);

    // Video dosyalarını filtrele
    const videoFiles = files.items.filter((item) =>
      /\.(mp4|avi|mkv)$/i.test(item.name) // Büyük/küçük harf uyumluluğu sağlandı
    );
    ...
    const videos = await Promise.all(
      videoFiles.map(async (video) => {
        ...
        const coverName = video.name.replace(/\.(mp4|avi|mkv)$/i, ".jpg");
        ...
        try {
          ...
          console.warn("Cover image not found:", coverName);
        }
