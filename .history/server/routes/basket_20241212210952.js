...
// İndirim kontrol ve uygulama fonksiyonu
async function applyDiscountIfEligible(userId, basketItems, totalAmount) {
  try {
    // Discount koleksiyonundan kullanıcıya ait belgeyi getir
    const discountSnapshot = await db
      .collection("discount")
      .where("userId", "==", userId)
      .get();

    // Şartları kontrol et: Discount belgesi ve basket uzunluğu
    if (!discountSnapshot.empty && basketItems.length >= 4) {
      const discountTotal = +(totalAmount * 0.8).toFixed(2); // %20 indirim uygula
      return { discountApplied: true, discountTotal };
    }

    // Şartlar sağlanmadıysa toplamı döndür
