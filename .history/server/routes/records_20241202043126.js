...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Check if "discount" collection exists for the user
    const discountCollection = await getQueryData("discount", "userId", userId);
    let discountTotal = 0;

    if (discountCollection.length > 0) {
      // If the discount collection exists, set discount_total
      discountTotal = userBasket.discount_total || 0; // Use basket's discount_total value if it exists
    }

    let discountMessage = discountCollection.length > 0
      ? `Discount applied. Total discount: ${discountTotal}`
      : "No discount applied.";
    ...
    const record = new Record(
      ...
      discountTotal, // Include discountTotal only if discount exists
      contact,
      payment_method,
      userBasket.items,
      new Date(),
      // Expiry date set to 16 days later
    ...
    // Clear the basket after the record is added
