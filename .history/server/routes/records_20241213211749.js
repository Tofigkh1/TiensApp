...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Find unique item IDs in the basket
    const uniqueItems = [...new Set(userBasket.items.map(item => item.product_id))];

    // Check if basket has 4 or more unique items
    if (uniqueItems.length >= 4) {
      const userDiscounts = await getQueryData("discount", "userId", userId);

      // Delete all discounts related to the user
