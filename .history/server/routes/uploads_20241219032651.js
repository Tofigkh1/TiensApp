...
export async function handlerGetAllVideos(req, res, folder) {
  try {
    ...
    // Videoları ve kapakları ID bazında eşle
    const mediaList = videoFiles.items.map((video) => {
      const videoId = video.name.split('.')[0]; // ID'yi dosya adından al
      const cover = coverFiles.items.find((cover) => cover.name.startsWith(videoId));
      
      if (cover) {
        return {
          id: videoId,
          videoUrl: getDownloadURL(video), // Asenkron çağrı olmaması için URL'yi doğrudan döndür
          coverImageUrl: getDownloadURL(cover),
        };
      }
      return null;
    }).filter(Boolean);
