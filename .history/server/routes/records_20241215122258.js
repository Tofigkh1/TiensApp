...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Firestore'dan kullanıcının sepetini getir
    const userBasket = await getQueryData("basket", "id", basket_id);
    ...
    // Firestore'da discount kontrolü
    const userDiscounts = await getQueryData("discount", "userId", userId);
    const hasExistingDiscount = userDiscounts.length > 0;

    // Eğer userId'ye ait basket items uzunluğu >= 4 ise ve indirim kodu varsa, sil
    if (userBasket.items.length >= 4 && hasExistingDiscount) {
