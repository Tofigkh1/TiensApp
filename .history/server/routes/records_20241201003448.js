...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Check for existing records and discounts
    ...
    // Silme işlemi: Kullanıcının "discount" koleksiyonundaki belgesini sil
    const userDiscount = await getQueryData("discount", "userId", userId);

    if (userDiscount.length > 0) {
      // Kullanıcıya ait birden fazla kayıt varsa hepsini sil
      for (const discount of userDiscount) {
        await deleteData("discount", discount.id);
        console.log(`Discount record deleted for userId ${userId}: ${discount.id}`);
      }
    }
