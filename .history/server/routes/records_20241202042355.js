...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Check if "discount" collection has records for the user
    const userDiscounts = await getQueryData("discount", "userId", userId);
    let discountTotal = 0;

    if (userDiscounts.length > 0) {
      // If there are discounts, include discount_total in the record
      discountTotal = userBasket.discount_total || 0;
    }
    ...
    // Create a new record with discount (if any)
    const record = new Record(
      ...
      discountTotal, // Use discount_total if applicable
      contact,
      payment_method,
      userBasket.items,
      new Date(),
