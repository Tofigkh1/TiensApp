...
export async function handlerGetAllVideos(req, res) {
  try {
    ...
    if (!files.items.length) {
      res.status(404).json({ error: "No videos found in the folder" });
      return;
    }
    ...
    if (!videoFiles.length) {
      res.status(404).json({ error: "No valid video files found" });
      return;
    }
    ...
    res.status(500).json({ error: "An unexpected error occurred while fetching videos" });
  }
}
