...
export async function handlerGetAllVideos(req, res, folder) {
  try {
    // Klasörlerin yollarını belirle
    const videosFolder = path.join(folder, "videos");
    const coversFolder = path.join(folder, "covers");

    // Video dosyalarını al
    const videoFiles = await fs.readdir(videosFolder);
    const videoUrls = videoFiles.map((file) => `${folder}/videos/${file}`);

    // Kapak resimlerini al
    const coverImageFiles = await fs.readdir(coversFolder);
    const coverImageUrls = coverImageFiles.map((file) => `${folder}/covers/${file}`);

    // Sonuçları döndür
    res.status(200).json({
      videos: videoUrls,
      covers: coverImageUrls,
    });
  } catch (error) {
