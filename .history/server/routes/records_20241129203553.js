...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    const userRecord = await getDataID("users", userId); // Kullanıcıyı temsil eden koleksiyon
    if (!userRecord) {
      return res.status(404).json({ error: "User not found!" });
    }

    // Kullanıcının indirim kodu daha önce oluşturulmuş mu?
    if (userRecord.discountCodeGenerated) {
      return res.status(400).json({ error: "Discount code already generated for this user." });
    }

    // Yeni indirim kodu oluştur
    const discountCode = uuidv4().replace(/-/g, "").slice(0, 12);

    // İndirim kodunu veritabanına ekle
    ...
    await uptData("users", userId, { discountCodeGenerated: true });

    // 16 gün sonrası için sona erme tarihi
    ...
    res.status(201).json({
      ...
      record: data,
      discount: `New discount code created: ${discountCode}`,
    });
  } catch (error) {
