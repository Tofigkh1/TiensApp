...
// Firestore'dan discount token'ını kontrol eden fonksiyon
async function checkDiscountToken(col, userId) {
  const discounts = await getQueryData(col, "user_id", userId);
  const discountToken = discounts?.[0]?.token; // Örneğin, token burada tutuluyorsa
  return discountToken ? true : false;
}
...
export async function handlerAddProductCardPOST(req, res, col, discountCol) {
  ...
  try {
    ...
    const hasDiscount = await checkDiscountToken(discountCol, decodedToken.userId);

    if (hasDiscount) {
      discountAmount = +(totalAmount * 0.2).toFixed(2); // %20 indirim
    }
    ...
    const newdData = {
      ...
      total_amount: totalAmount - discountAmount, // İndirim uygulanmış toplam tutar
      discount_amount: discountAmount, // İndirim miktarı
    };
