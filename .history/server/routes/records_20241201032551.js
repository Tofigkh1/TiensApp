...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    if (userBasket.items.length >= 4) {
      discountTotal = userBasket.total_amount * 0.1; // Example: 10% discount
    }
    ...
    const record = new Record(
      ...
      email,
      delivery_address,
      userBasket.total_amount - discountTotal,
      contact,
      payment_method,
      userBasket.items,
      new Date(),
    ...
    // Clear basket and reset total_amount and total_count, including discount_total
    await uptData("basket", basket_id, { 
      items: [], 
      total_amount: 0, // Reset total amount
      total_count: 0,  // Reset total count if applicable
      discount_total: 0 // Reset discount total
    });
    ...
    res.status(201).json({
      ...
      discount_total: discountTotal, // Return discount_total for confirmation
    });
  } catch (error) {
