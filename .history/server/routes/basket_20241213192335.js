...
async function applyDiscountIfAvailable(userId, totalAmount, totalItems) {
  try {
    ...
    console.log("Discount Snapshot:", discountSnapshot.size);
    ...
    if (!discountSnapshot.empty && totalItems >= 4) {
      ...
      if (discountCode) {
        ...
        console.log("Discount Applied. New Total:", discountTotal);
...
export async function handlerDeleteProductCard(req, res, col) {
  ...
  try {
    ...
    let discountTotal = totalAmount;

    if (totalItems >= 4) {
      const discountResult = await applyDiscountIfAvailable(
        decodedToken.userId,
        totalAmount,
        totalItems
      );
    ...
    const newdData = {
      ...
      user_id: decodedToken.userId,
      total_amount: totalAmount,
      ...(discountApplied && { discount_total: discountTotal }),
    };
