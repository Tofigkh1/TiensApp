...
async function applyDiscountIfAvailable(userId, totalAmount, uniqueItemCount) {
  try {
    ...
    if (!discountSnapshot.empty && uniqueItemCount >= 4) {
...
export async function handlerDeleteProductCard(req, res, col) {
  ...
  try {
    ...
    const uniqueItemCount = newItems.length; // Farklı ürün sayısı
    const totalItems = newItems.reduce((sum, item) => sum + item.count, 0);
    ...
    const { discountApplied, discountTotal } = await applyDiscountIfAvailable(
      decodedToken.userId,
      totalAmount,
      uniqueItemCount // Burada farklı ürünlerin sayısını gönderiyoruz
    );

    const newdData = {
      items: newItems,
      total_count: totalItems,
      total_item: uniqueItemCount, // Farklı ürün sayısı
      user_id: decodedToken.userId,
      total_amount: totalAmount,
      discount_total: discountApplied ? discountTotal : 0, // İndirim uygulanmazsa 0
    };
