...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Check if a discount exists for the user
    const userDiscounts = await getQueryData("discount", "userId", userId);

    if (userBasket.items.length >= 4) {
      // If basket has 4 or more items, delete existing discounts
      ...
      console.log(`Basket does not meet the criteria for discount deletion. Items count: ${userBasket.items.length}`);
    }
