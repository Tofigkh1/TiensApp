...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Firestore'da discount koleksiyonunun varlığını kontrol et
    const discountCollectionExists = await getAllData("discount");

    // Süre sonu tarihini belirle
    ...
    const record = new Record(
      ...
      undefined, // Başlangıçta indirim yok
      contact,
      payment_method,
      userBasket.items,
      new Date(),
    ...
    // Eğer discount koleksiyonu varsa discount_total ekle
    if (discountCollectionExists.length > 0) {
      record.discount_total = userBasket.discount_total || 0;
    }

    const data = await addData(col, record);

    // Sepeti temizle ve total_amount ile total_count'u sıfırla
    await uptData("basket", basket_id, { 
      items: [], 
      total_amount: 0, // Toplam miktarı sıfırla
      total_count: 0  // Toplam sayıyı sıfırla
    });
