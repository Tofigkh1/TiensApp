...
async function applyDiscountIfAvailable(userId, totalAmount) {
  try {
    const discountSnapshot = await db
      .collection("discount")
      .where("userId", "==", userId) // Kullanıcıya özel indirim kodu varsa kontrol eder
      .get();
    ...
    return { discountApplied: false, discountTotal: totalAmount }; // İndirim yok
  } catch (error) {
...
//     // Eğer ürünler 4'ten azsa discountTotal 0 olarak döner
//     return { discountApplied: false, discountTotal: 0 };
//   } catch (error) {
//     console.error("Error checking discount:", error);
//     return { discountApplied: false, discountTotal: totalAmount }; // Varsayılan olarak totalAmount döner
//   }
// }
