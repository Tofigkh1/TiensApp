...
async function applyDiscountIfAvailable(userId, totalAmount) {
  try {
    const discountCollectionSnapshot = await db.collection("discount").get();

    if (discountCollectionSnapshot.empty) {
      // Discount koleksiyonu yoksa indirim uygulanmaz
      return { collectionExists: false, discountApplied: false, discountTotal: totalAmount };
    }
    ...
    if (!discountSnapshot.empty) {
      ...
      if (discountCode) {
        ...
        return { collectionExists: true, discountApplied: true, discountTotal };
      }
    }

    return { collectionExists: true, discountApplied: false, discountTotal: totalAmount };
  } catch (error) {
...
export async function handlerAddProductCardPOST(req, res, col) {
  ...
  try {
    ...
    const { collectionExists, discountApplied, discountTotal } = await applyDiscountIfAvailable(
      decodedToken.userId,
      totalAmount
    );

    // Koleksiyon varsa ve indirim uygulanmışsa discount_total eklenir
    const newdData = {
      items: newItems,
      ...
      ...(collectionExists && discountApplied && { discount_total: discountTotal }),
    };
