...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Check if discount collection exists
    const discountExists = await getAllData("discount");
    let discountTotal = null;

    if (discountExists && discountExists.length > 0) {
      discountTotal = userBasket.discount_total || 0; // Use discount_total only if discount collection exists
    }
    ...
    const record = new Record(
      ...
      discountTotal, // Only include discountTotal if discount collection exists
      contact,
      payment_method,
      userBasket.items,
      new Date(),
