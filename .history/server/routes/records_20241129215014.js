...
export async function handlerAddRecordPOST(req, res, col) {
  ...
  try {
    ...
    // Check if the user already has a discount code in records collection
    const existingRecord = await getQueryData(ROUTER.RECORDS, "userId", userId);

    // Check the noDiscount collection for hasDiscount field
    ...
    if (!existingRecord.length && !hasExistingDiscount) {
      // Check if basket contains 4 or more items before generating a discount code
      if (userBasket.items.length >= 4) {
        const discountCode = uuidv4().replace(/-/g, "").slice(0, 12);

        // Add the discount code to the "discount" collection
    ...
    // 16 days later expiry timestamp
