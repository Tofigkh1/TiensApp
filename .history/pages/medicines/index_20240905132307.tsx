...
function Medicines() {
  ...
  const handleMouseEnter = () => {
    setIsHovered(true);  // Fare açılan ürünlerin üzerinde
  };

  const handleMouseLeave = () => {
    setIsHovered(false);  // Fare açılan ürünlerden ayrıldı
    setHoveredCategory(null);  // Eğer fare açılan ürünlerden de ayrılırsa kategori de sıfırlanır
  };
    ...
    <div>
      ...
      {hoveredCategory && (
          ...
          {getProductsByCategory(hoveredCategory)?.map((product: any) => (
            <Box key={product.id} borderWidth="1px" borderRadius="full" p={3} textAlign="center" width="100px" height="100px">
              <Image width={80} height={80} src={product?.img_url} alt={product.name} borderRadius="full" boxSize="50px" />
              <Text fontSize="sm" mt={2}>{product.name}</Text>
            </Box>
          ))}
