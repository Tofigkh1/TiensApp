...
import { checkDiscountCollection } from '../../../../server/configs/firebaseAdmin';
...
// discount_total
  ...
  const basketDiscount = basket?.data?.discount_total;

  console.log("discount_total",basketDiscount);
...
useEffect(() => {
  ...
  const handleCheckout = async (values: FormValues) => {
    try {
      // Firestore'da discount collection'ı kontrol et
      const hasDiscountCollection = await checkDiscountCollection();
  
      // Discount collection varsa discount_total'i dahil et, yoksa dahil etme
      const RecordsData: any = {
        id: "",
        user_id: user.id ?? "",
        basket_id: basketIdData,
        contact: values.phoneNumber, // Kullanıcıdan alınan telefon numarası
        delivery_address: values.address, // Kullanıcıdan alınan adres
        fullname: user.fullname,
        payment_method: "Bank Kartlı ilə ödəmə", // Ödeme yöntemi
        email: user.email,
        date: new Date().toISOString(),
        amount: basketAmount ?? 0,
        created: new Date().toISOString(),
        price: 0,
        // Sadece discount collection varsa discount_total eklenir
        ...(hasDiscountCollection
          ? { discount_total: parseFloat(String(basketDiscount ?? 0)) }
          : {}),
      };
      ...
      toast({
        title: "An error occurred while checking the discount collection",
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "top-right",
