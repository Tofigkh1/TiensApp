...
const VideoPlayer = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        // API'den videoları getir
        const response = await axios.get("/api/uploads?folder=uploads&all=true");
        console.log("response", response);

        // Backend'den dönen video URL'lerini düzenle
        const responseVideos = response.data.videos.map(
          (video) => `/uploads/videos/${video.videoUrl}` // Video URL'lerini tam path ile oluştur
        );
