...
export default function ProductsCard(product: ProductState) {
    const { id, description, img_url, name, price } = product;
    ...
    const handleDeleteFromBasket = () => {
        ...
        const basketProduct = basket?.items?.find((basketProduct: any) => basketProduct?.product_id === id);
        const ageSize = basketProduct?.ageSize;

        if (!ageSize) {
            toast({
                title: "Product size is not selected",
                status: 'error',
                duration: 2000,
                isClosable: true,
                position: 'top-right',
        ...
        dispatch(deleteFromBasket(deleteProduct)).then((action) => {
            if (action.type === deleteFromBasket.rejected.type) {
                ...
                dispatch(fetchBasket());
                setButtonClicked(false);
                dispatch(resetButtonVisibility());
                toast({
