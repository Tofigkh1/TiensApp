...
export default function Search() {
    ...
    const handleSearchClick = () => {
        // LocalStorage'den arama geçmişini al
        const storedHistory = localStorage.getItem('searchHistory');
        const parsedHistory = storedHistory ? JSON.parse(storedHistory) : [];
        
        // LocalStorage'den uygun ürünü bul
        const matchedProductFromHistory = parsedHistory.find(
            (item: { name: string }) => item.name?.toLowerCase() === query.toLowerCase()
        );
    
        if (matchedProductFromHistory) {
            // Eğer geçmişte eşleşen ürün varsa yönlendir
            push('/medicines/' + matchedProductFromHistory.id);
            setAlert(false);
    
            // Arama geçmişini güncelle
            const updatedHistory = [
                matchedProductFromHistory,
                ...parsedHistory.filter(
                    (item: { name: string }) =>
                        item.name?.toLowerCase() !== matchedProductFromHistory.name?.toLowerCase()
                ).slice(0, 4),
            ];
            setSearchHistory(updatedHistory);
            localStorage.setItem('searchHistory', JSON.stringify(updatedHistory));
        } else {
            // Eğer geçmişte eşleşen ürün yoksa hata ver
            setAlert(true);
        }
    };
