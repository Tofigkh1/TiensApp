...
export default function Search() {
    ...
    const [productMap, setProductMap] = useState<Map<string, ProductPostDataType>>(new Map());
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const fetchedProducts = response?.data?.result?.data.map((product: PostDataType): ProductPostDataType => ({
                    ...product,
                ...
                setProductMap(new Map(fetchedProducts.map(p => [p.name.toLowerCase(), p])));
            } catch (error) {
    ...
    const matchedProduct = productMap.get(query.toLowerCase());
