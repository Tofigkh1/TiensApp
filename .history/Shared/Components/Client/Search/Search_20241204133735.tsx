...
export default function Search() {
    ...
    const handleSearchClick = () => {
        ...
        if (query) {
            let updatedHistory = searchHistory.filter(item => item.name.toLowerCase() !== query.toLowerCase()); // Remove if already exists
            updatedHistory = [{ id: selectedProduct?.id ?? null, name: query }, ...updatedHistory.slice(0, 4)]; // Add to top and limit to 5 items
        ...
        <ul>
            {searchHistory.map((historyItem, index) => (
                <li className="w-96" key={index} onClick={() => {
                    setQuery(historyItem.name); // Geçmiş öğesinin adını input'a yaz
                    setFocus(false);           // Fokus geçmiş listesi kapansın
                    if (historyItem.id) {
                        push('/medicines/' + historyItem.id); // ID varsa ilgili sayfaya yönlendir
                    }
                }}>
