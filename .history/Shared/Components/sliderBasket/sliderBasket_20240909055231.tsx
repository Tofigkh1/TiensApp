  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dataImageUrl = dataResult?.data?.result?.data?.items[0].img_url;
        setImageUrl(dataImageUrl);

        console.log("Fetched image URL:", dataImageUrl);
      ...
      <div className={`${styles.menu} ${isMenuOpen ? styles.menuOpen : styles.menuClosed}`}>
        <div ref={menuRef} className={`${styles.menu} ${isMenuOpen ? styles.menuOpen : styles.menuClosed}`}>
          ...
          <button onClick={handleToggleMenu} className={styles.buyButton}>
            ...
            {/* Burada URL'yi bir resim olarak render ediyoruz */}
            {imageUrl && <Image src={imageUrl} alt="Product Image" width={50} height={50} />}
          </button>
        </div>
      </div>
    </>
