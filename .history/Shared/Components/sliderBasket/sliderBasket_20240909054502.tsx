...
const BasketMenu = () => {
  ...
  const productsImages = basket.data.items[1].img_url;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dataCountImage = dataResult.data.result.data.items.img_url 
        setCount(dataCountImage)

        console.log("dataResult",dataResult.data.result.data.items.img_url       );
