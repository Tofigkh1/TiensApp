...
const BasketMenu = () => {
  ...
  // Sepet öğeleri değiştiğinde güncelle, eğer veri null değilse
  useEffect(() => {
    if (basketItems && basketItems.length > 0) {
      const imgUrls = basketItems.map((item: any) => item.img_url);
      setImageUrl(imgUrls);
    } else {
      setImageUrl([]); // Eğer sepet boşsa boş bir dizi atayarak eski veriyi temizle
    }
  }, [basketItems]);

  // Sayfa geçişleri sırasında render problemi yaşanmasın diye olay dinleyici kullanımı
  useEffect(() => {
