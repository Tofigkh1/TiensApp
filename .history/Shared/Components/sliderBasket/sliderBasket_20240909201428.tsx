...
const BasketMenu = () => {
  ...
  // URL değişikliklerini izlemek için useEffect
  useEffect(() => {
    const handleRouteChange = () => {
      const imgUrls = basketItems?.length > 0 
        ? basketItems.map((item: any) => item.img_url) 
        : [];
      setImageUrl(imgUrls);
    };
    ...
    // Route değişikliği izleyicisini temizle
    return () => {
  ...
  }, [router, basketItems]); // Hem router hem de basketItems değişikliklerini izler
  

  // Sepete ürün eklenince otomatik güncelle
  useEffect(() => {
    const imgUrls = basketItems.map((item: any) => item.img_url);
    setImageUrl(imgUrls);
  }, [basketItems]);  // basketItems değişikliklerini izler

  // Fetch basket data when component mounts
  useEffect(() => {
          ...
          {basketItems.length > 0 ? (
            <div>
              {imageUrl.map((url, index) => (
                <div key={url} className="w-36 h-36 h-auto m-4 rounded-2xl border border-whiteLight3 bg-white">
                  <div className="mySwiper cursor-pointer">
                    <Image key={index} src={url} alt={`Product Image ${index + 1}`} width={120} height={120} />
                  </div>
                </div>
              ))}
            </div>
